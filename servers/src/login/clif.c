...
int clif_parse(int fd) {
	...
	case 0x02:
		...
		if ((RFIFOB(fd, 5) > 15) || (RFIFOB(fd, 5) < 3) || string_check_allchars(RFIFOP(fd, 6),RFIFOB(fd, 5))) {
			clif_message(fd, 0x03, login_msg[LGN_ERRUSER]);
			break;
		}
		
		if ((RFIFOB(fd, 6+RFIFOB(fd, 5)) > 8) || (RFIFOB(fd, 6+RFIFOB(fd, 5)) < 6) || string_check(RFIFOP(fd, 7+RFIFOB(fd, 5)),RFIFOB(fd, 6+RFIFOB(fd, 5)))) {
		...
		if(strlen(sd->pass)>8 || strlen(sd->pass)<6)  {
			clif_message(fd,0x05, login_msg[LGN_ERRPASS]);
			break;
		}
		if (char_fd) {
	...
	case 0x03:
		...
		if(require_reg) {
			if (!reg_check(RFIFOP(fd,6),RFIFOB(fd,5))) {
				clif_message(fd,0x03, "You must register your character on mYnexia Website!");
				break;
			}
		}
		memcpy(sd->name,RFIFOP(fd,6),RFIFOB(fd,5));
