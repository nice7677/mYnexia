...
char* itemdb_commonscript(unsigned int id) {
	struct item_data *db;
	db=itemdb_search(id);
	return db->commonscript;
}
...
int itemdb_read(const char *itemdb_file) {
	...
	memset(&item,0,sizeof(item));
		...
		if(SQL_ERROR==SqlStmt_Prepare(stmt,"SELECT `ItmId`, `ItmDescription`, `ItmIdentifier`, `ItmMark`, `ItmType`, `ItmBuyPrice`, `ItmSellPrice`, `ItmMaximumAmount`, `ItmPthId`, `ItmSex`, `ItmLevel`, `ItmLook`, "
			...
			"`ItmThrown`, `ItmUniqueAmount`, `ItmIndestructible`, `ItmTimer`, `ItmRepairable`, `ItmCommonScript` FROM `Items`")
		...
		|| SQL_ERROR==SqlStmt_BindColumn(stmt,43,SQLDT_UINT,&item.repairable,0,NULL,NULL)
		|| SQL_ERROR==SqlStmt_BindColumn(stmt,44,SQLDT_STRING,&item.commonscript,sizeof(item.commonscript),NULL,NULL)
		)
			{
				SqlStmt_ShowDebug(stmt);
				SqlStmt_Free(stmt);
