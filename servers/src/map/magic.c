...
int magicdb_read() {
	struct magic_data *db = NULL;
	int i, mag, id;
	unsigned char type, dispell, aether, mute, ticker;
	char class;
	...
	if(SQL_ERROR == SqlStmt_Prepare(stmt,"SELECT `SplId`, `SplDescription`, `SplIdentifier`, `SplType`, `SplQuestion`, `SplDispel`, `SplAether`, `SplMute`, `SplPthId`, `SplNoTicker` FROM `Spells` WHERE `SplActive` = '1'")
	...
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 9, SQLDT_CHAR, &ticker,0,NULL,NULL)
	)
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
