...
int clif_object_look_sub(struct block_list *bl, va_list ap) {
	...
	case BL_ITEM:
		...
		if (itemdb_type(item->data.id) == ITM_TRAPS && !(sd->spottraps)) {
            //printf("1. ownerid %u | userid: %u \n", item->data.owner_id, sd->bl.id);
            nd=(NPC *)map_firstincell(item->bl.m, item->bl.x, item->bl.y, BL_NPC);
            nullpo_ret(0, nd);
            //printf("1. npcownerid %u | npcname: %u \n", nd->owner, nd->name);
            if (sd->bl.id != nd->owner)//if trap active and owner not same then dont show trap
            {
                return 0;
            }
		}
...
int clif_object_look_sub2(struct block_list *bl, va_list ap) {
	...
	case BL_ITEM:
		...
		if (itemdb_type(item->data.id) == ITM_TRAPS && !(sd->spottraps)) {
            //printf("2. ownerid %u | userid: %u \n", item->data.owner_id, sd->bl.id);
            nd=(NPC *)map_firstincell(item->bl.m, item->bl.x, item->bl.y, BL_NPC);
            nullpo_ret(0, nd);
            //printf("2. npcownerid %u | npcname: %u \n", nd->owner, nd->name);
            if (sd->bl.id != nd->owner)//if trap active and owner not same then dont show trap
            {
                return 0;
            }
...
int clif_send_duration(USER* sd,int id,int time, USER* tsd) {
	...
	//new function to show ticker or not 0 show 1 no show
	if(magicdb_ticker(id) && time > 0)
	{
		time = -1;//-1 no display timer tick just like etk :D
	}
...
int clif_send_mob_health(MOB* mob, int damage, int critical){
	...
	if(mob->data->subtype==0) {
        ...
        if(strcmpi(mob->data->commonscript,"") == 0) {
            sl_doscript_blargs(mob->data->yname,"on_attacked",2,&mob->bl,bl);
        } else {
            sl_doscript_blargs(mob->data->commonscript,"on_attacked",2,&mob->bl,bl);
        }
	} else if(mob->data->subtype == 5) {
		sl_doscript_blargs("mob_ai_ghost","on_attacked",2,&mob->bl,bl);
	} else if(mob->data->subtype == 6) {
		sl_doscript_blargs("mob_ai_pet","on_attacked",2,&mob->bl,bl);
	}
...
int clif_send_mob_healthscript(MOB* mob, int damage, int critical) {
	...
	if (!mob->current_vita) {
        if(strcmpi(mob->data->commonscript,"") == 0) {
            ...
            sl_doscript_blargs(mob->data->commonscript,"before_death",2,&mob->bl,bl);
        }
	...
	if (!mob->current_vita) {
		...
		if(sd != NULL && mob->summon == 0) {
			...
			if(sd->group_count == 0) {
				pc_givexp(sd, mob->data->exp, xp_rate, 0);
        ...
        if(strcmpi(mob->data->commonscript,"") == 0) {
            ...
            sl_doscript_blargs(mob->data->commonscript, "after_death", 2, &mob->bl, bl);
        }
	}
...
int clif_sendstatus(USER* sd, int flags) {
	...
	if(level<99) {
		helper=classdb_level(path,level); //status bar exp
		helper-=classdb_level(path,level-1);
		tnl=classdb_level(path,level)-(sd->status.exp); //status bar exp
		percentage=(((float)(helper-tnl))/(helper))*100; //status bar exp
		//printf("Helper: %d\n",helper);
		//printf("Tnl: %d\n",tnl);		 
	} else {
...
int clif_parsewalk(USER *sd) {
	...
	if ((speed != sd->speed || (speed != 90 && sd->status.state != 3)) && !sd->status.gm_level)
		//printf("(Walk edit) Name: %s Speed Sent: %d, Char Speed: %d\n", sd->status.name, speed, sd->speed);
	...
	*/
	...
	sl_doscript_blargs("onWalk", NULL, 1, &sd->bl);
	return 0;
}
...
int clif_sendgroupmessage(USER *sd, unsigned char* msg, int msglen) {
	...
	if (classdb_name(sd->status.class, sd->status.mark)) {
		sprintf(buf2,"<PARTY>%s(%s): %s",sd->status.name,classdb_name(sd->status.class,sd->status.mark), message);
	} else {
		sprintf(buf2, "<PARTY>%s(%s): %s", sd->status.name, message);
	}

	for (i = 0; i < sd->group_count; i++) {
		tsd = map_id2sd(groups[sd->groupid][i]);
		if (tsd && clif_isignore(sd, tsd)) {
			clif_sendmsg(tsd,0,buf2,strlen(buf2));
		}
	}
	return 0;
}
...
int clif_sendreportmessage(USER *sd, unsigned char* msg, int msglen) {
	char message[256];
	char buf[65535];
	USER* tsd = NULL;
	int i;
	...
	if (classdb_name(sd->status.class, sd->status.mark)) {
		sprintf(buf,"<REPORT>%s(%s): %s",sd->status.name,classdb_name(sd->status.class,sd->status.mark), message);
	} else {
		sprintf(buf, "<REPORT>%s(%s): %s", sd->status.name, message);
	}
	
	for(i=0;i<fd_max;i++) {
		if(session[i] && (tsd=session[i]->session_data)) {
			if(tsd->status.gm_level >= 1) {
...
int clif_sendclanmessage(USER *sd,unsigned char* msg, int msglen) {
	...
	if (classdb_name(sd->status.class, sd->status.mark)) {
		sprintf(buf2,"<CLAN>%s(%s): %s",sd->status.name,classdb_name(sd->status.class,sd->status.mark), message);
	} else {
		sprintf(buf2, "<CLAN>%s(%s): %s", sd->status.name, message);	//	Clan 
	}
	/*WBUFB(buf,0)=0xAA;
	...
	for(i=0;i<fd_max;i++) {
		if(session[i] && (tsd=session[i]->session_data) && clif_isignore(sd, tsd)) {
			if(tsd->status.clan_id==sd->status.clan_id) {
				...
				clif_sendmsg(tsd,0,buf2,strlen(buf2));	//Clan msg
				}
			}
		}
...
int clif_parsewisp(USER *sd) {
	...
	{
        clif_sendbluemessage(sd,"Silenced!");
		return 0;
	}
...
*/
	...
	if(!strcmp(dst_name,"!")) {
		...
		if(SQL_ERROR == Sql_Query(sql_handle,"INSERT INTO `ReportLogs` (`RepChaId`, `RepMessage`) VALUES ('%u', '%s')",
			sd->status.id, escape)) {
			SqlStmt_ShowDebug(sql_handle);
		}
		clif_sendreportmessage(sd,RFIFOP(sd->fd,7+dstlen),msglen);
	} else if(!strcmp(dst_name,"@")) {
	...
	if (!dst_sd) {
			sprintf(strText,"%s is not online..",dst_name);
...
int clif_sendmobsay(struct block_list* bl, va_list ap) {
    ...
    if(strcmpi(mob->data->commonscript,"") == 0) {
        ...
        sl_doscript_blargs(mob->data->commonscript, "say", 2, &sd->bl, &mob->bl);
    }
	return 0;
}
...
int clif_sendmobyell(struct block_list* bl, va_list ap) {
    ...
    if(strcmpi(mob->data->commonscript,"") == 0) {
        ...
        sl_doscript_blargs(mob->data->commonscript, "yell", 2, &sd->bl, &mob->bl);
    }
	return 0;
}
...
int clif_parsemagic(USER* sd) {
	...
	if (i > 0) {
		...
		len = sprintf(msg, "Wait %d second(s) to settle.", time);
...
int clif_parseattack(USER *sd) {
	...
	if(itemdb_thrown(sd->status.equip[EQ_WEAP].id)) {// && sd->time < 3) {
		...
		if(itemdb_sound(sd->status.equip[EQ_WEAP].id)==0) {
			clif_sendaction(&sd->bl,1,speed,710);				//Swing sound effect
			...
			case BL_MOB:
				...
				if (CheckProximity(one, two, Radius) == 1) {
                    ...
                    if(strcmpi(mob->data->commonscript,"") == 0) {
                        ...
                        sl_doscript_blargs(mob->data->commonscript, "click", 2, &sd->bl, &mob->bl);
                    }
				}
...
int clif_handle_boards(USER* sd) {
	...
	switch(RFIFOB(sd->fd,5)) {
		...
		case 4: //Make post
			...
			sl_doscript_blargs("onUpdateBoard", NULL, 1, sd->bl);
			break;
		case 5: //delete post!
		...
		//sl_doscript_blargs("onViewBoard", NULL, 1, &sd->bl);
		clif_sendaction(&sd->bl, 15, 0x4E, 0);	//kirim action boring kalo baca/lihat boards
...
int clif_parse(int fd) {
	...
	//printf("Case %u | RFIFOB-5: %u | RFIFOB-6: %u \n", RFIFOB(fd, 3), RFIFOB(sd->fd,5), RFIFOB(sd->fd,6));
	switch (RFIFOB(fd, 3)) {
	case 0x05:
		clif_cancelafk(sd);
	...
	case 0x13:
		...
		if (sd->attacked != 1 && sd->attack_speed > 0) {
			...
			if(map[sd->bl.m].special > 0)//if on special map :D
			{
				sl_doscript_blargs("onSpecialMapAttack", NULL, 1, &sd->bl);
			}
			else
			{
				clif_parseattack(sd);//if 0 normal attack
			}
...
//Packet 45 responds from 3B
	...
	case 0x83: //screenshots...
		clif_cancelafk(sd);
		sl_doscript_blargs("onScreenShoot", NULL, 1, &sd->bl);
		break;
	default:
		//printf("Unknown Packet ID: %02X Packet content:\n",RFIFOB(sd->fd, 3));	//disabled print unknown packet
		clif_debug(RFIFOP(sd->fd, 5), SWAP16(RFIFOW(sd->fd, 1)) - 5);
		//if (dump_save)
		//	add_dmp(fd, len);
		break;
...
int clif_groupstatus(USER *sd) {
	...
	for (x = 0, n = 0, w = 0, r = 0, m = 0, p = 0, a = 0, g = 0; (n + w + r + m + p + a + g) < sd->group_count; x++) {
		...
		if (!pc_isequip(tsd,EQ_HELM) || !(tsd->status.settingFlags & FLAG_HELM) || (itemdb_look(pc_isequip(tsd,EQ_HELM))==-1)) {
			...
			WFIFOW(sd->fd, len+8)=0x0; // supposed to be Helm num
			WFIFOB(sd->fd, len+10)=0;
		} else {
			WFIFOB(sd->fd, len+7)=1;
...
int clif_groupexp(USER *sd, unsigned int xp) {
	...
	for (x = 0; x < sd->group_count; x++) {
		...
		if(tsd->bl.m == sd->bl.m && tsd->status.state != 1) {
			...
			pc_givexp(tsd, (group_xp * percentage) + 1, xp_rate, 0);
		}
	}
	sl_doscript_blargs("onGroupXP", NULL, 2, &sd->bl, &tsd->bl);
	return 0;
}
...
int clif_leavegroup(USER *sd) {
	...
	sprintf(buff,"You have left the party.");
	clif_sendminitext(sd,buff,strlen(buff));
	sd->group_count=0;
	sd->groupid = 0;
	sl_doscript_blargs("onLeaveGroup", NULL, 1, &sd->bl);
}
int clif_findmount(USER* sd) {
    ...
    if(strcmpi(mob->data->commonscript,"") == 0) {
        ...
        sl_doscript_blargs(mob->data->commonscript, "on_mount", 2, &sd->bl, &mob->bl);
    }
	return 0;
}
...
int clif_changestatus(USER *sd,int type) {
	int oldm, oldx, oldy;
	char buff[256];
	char summonedMount[] = "summoned_mount";
	int mountID = 0;
	...
	case 0x00: //Ride/something else
		if (RFIFOB(sd->fd,7) == 1) {
			if(sd->status.state == 0 || sd->status.state == 2) {
				if(map[sd->bl.m].special > 0) {
					sl_doscript_blargs("onSpecialMapR", NULL, 1, &sd->bl);
				}
				else {
					mountID = pc_readglobalreg(sd, summonedMount);
					if(mountID > 0) {
						sl_doscript_blargs("onSummonedMount", NULL, 1, &sd->bl);
					}
					else {
						clif_findmount(sd);
						if(sd->status.state==0) {
							clif_sendminitext(sd,"There is nothing here that you can ride.",0);
						}
					}
				}
	...
	case 0x01: //Whisper (F5)
        ...
        if(sd->status.settingFlags & FLAG_WHISPER) {
            clif_sendminitext(sd,"Whisper: ON",0);
        } else {
            clif_sendminitext(sd,"Whisper: OFF",0);
        }
        clif_sendstatus(sd,NULL);
        break;
	case 0x02: //group
		...
		if(sd->status.settingFlags & FLAG_GROUP) {
			sprintf(buff,"Group  : ON");
		} else {
			if(sd->group_count>0) {
				clif_leavegroup(sd);
			}
			sprintf(buff,"Group  : OFF");
		}
		clif_sendstatus(sd,NULL);
		clif_sendminitext(sd,buff,strlen(buff));
		break;
	case 0x03: //Shout
		...
		if (sd->status.settingFlags & FLAG_SHOUT) {
			clif_sendminitext(sd, "Shout  : ON", 0);
		} else {
			clif_sendminitext(sd, "Shout  : OFF", 0);
		}
		clif_sendstatus(sd,NULL);	
		break;	
	case 0x04: //Advice
		...
		if (sd->status.settingFlags & FLAG_ADVICE) {
			clif_sendminitext(sd, "Advice : ON", 0);
		} else {
			clif_sendminitext(sd, "Advice : OFF", 0);
		}
		clif_sendstatus(sd,NULL);
		break;
	case 0x05: //Magic
		...
		if(sd->status.settingFlags & FLAG_MAGIC) {
			clif_sendminitext(sd,"Magic  : ON",0);
		} else {
			clif_sendminitext(sd,"Magic  : OFF",0);
		}
		clif_sendstatus(sd,NULL);
		break;
	case 0x06: //Weather
		...
		if(sd->status.settingFlags & FLAG_WEATHER) {
				sprintf(buff, "Weather: ON");
			} else {
				sprintf(buff, "Weather: OFF");
			}
			clif_sendminitext(sd,buff,strlen(buff));
    ...
    case 0x07: //F4 key
		...
		if (sd->status.settingFlags & FLAG_REALM) {
			clif_sendminitext(sd, "Centered   :ON", strlen("Centered   :ON"));
		} else {
			clif_sendminitext(sd, "Centered   :OFF", strlen("Centered   :OFF"));
		}
		clif_sendstatus(sd,NULL);
		*/
		sl_doscript_blargs("onF4Key", NULL, 1, &sd->bl);
		break;
		
	case 0x08: //exchange
		//sd->exchange_on^=1;
		sl_doscript_blargs("onTradeKey", NULL, 1, &sd->bl);
...
/*
		...
		clif_sendstatus(sd,NULL);
*/
		sl_doscript_blargs("onF12Key", NULL, 1, &sd->bl);
		break;
	case 10: //Clan chat
...
int clif_parse_exchange(USER* sd) {
	...
	case 2: //after ask amount
		id=RFIFOB(sd->fd,10)-1;
		amount=RFIFOB(sd->fd,11);//unsignd char max is 255
       //amount=SWAP16(RFIFOW(sd->fd, 11));//test change to short so can hold more amount -.- not working i think is client designed -.-
		tsd=map_id2sd(SWAP32(RFIFOL(sd->fd,6)));
...
int clif_startexchange(USER* sd, unsigned int target) {
	...
	if(target==sd->bl.id) {
		sprintf(buff,"You move your items from one hand to another.");
		clif_sendminitext(sd,buff,strlen(buff));
		return 0;
	}
...
int clif_cancelafk(USER* sd) {
	...
	if (reset) {
		...
		*/
		// No need to insert un AFK logs into mySQL
