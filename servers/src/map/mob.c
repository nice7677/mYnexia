...
int mobdb_read(const char *mobdb_file) {
	...
	if(SQL_ERROR==SqlStmt_Prepare(stmt,"SELECT `MobId`, `MobDescription`, `MobIdentifier`, `MobBehavior`, `MobAI`, `MobLook`, `MobLookColor`, `MobVita`, `MobMana`, `MobExperience`, `MobHit`, `MobLevel`, `MobMight`,"
		...
		"`MobMark`, `MobIsNpc`, `MobCommonScript`, `MobCon`, `MobWisdom`, `MobMountLook` FROM `Mobs`")
	...
	|| SQL_ERROR==SqlStmt_BindColumn(stmt,58,SQLDT_STRING,&a.commonscript,sizeof(a.commonscript),NULL,NULL)
	|| SQL_ERROR==SqlStmt_BindColumn(stmt,59,SQLDT_UINT, &a.con,0,NULL,NULL)
	|| SQL_ERROR==SqlStmt_BindColumn(stmt,60,SQLDT_UINT, &a.wisdom,0,NULL,NULL)
	|| SQL_ERROR==SqlStmt_BindColumn(stmt,61,SQLDT_UINT, &a.mount_look,0,NULL,NULL)
    ){
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
		return 0;
...
int mobspawn_read(const char *mobspawn_file) {
	...
	for(i=0;i<mstr && SQL_SUCCESS==SqlStmt_NextRow(stmt);i++){
        //add checking mobexist before spawn
        if (!mobdb_searchexist(mob.mobid)) {
            mc++;
            printf("%d Spawn# %d Mob# %d not exist so skip spawning it \n", mc, mob.id, mob.mobid);
            continue;
        }
	...
	srand( gettick() );
	printf("Monster spawn read done. %u monsters spawned! (%u skipped)\n", mstr, mc);
	return 0;
}
		...
		if((mob->time>=mob->data->movetime && mob->time >= mob->newmove) || (mob->time >= mob->newmove && mob->newmove > 0)){
			...
			if(bl != NULL) {
				if(bl->type == BL_MOB) {
					...
					if ((sd->status.state == 1) || (sd->status.state == 2)) {
						mob->target = 0;
						mob->attacker = 0;
						bl = NULL;
					}
			...
			if(mob->data->subtype==0) {
                ...
                if(strcmpi(mob->data->commonscript,"") == 0) {
                    ...
                    sl_doscript_blargs(mob->data->commonscript,"move",2,&mob->bl,bl);
                }
			} else if(mob->data->subtype == 5) {
				sl_doscript_blargs("mob_ai_ghost","move",2,&mob->bl,bl);
			} else if(mob->data->subtype == 6) {
				sl_doscript_blargs("mob_ai_pet","move",2,&mob->bl,bl);
			}
		}
		...
		if((mob->time >= mob->data->atktime && mob->time >= mob->newatk) || (mob->time >= mob->newatk && mob->newatk > 0)) {
			...
			if(mob->data->subtype==0) {
                ...
                if(strcmpi(mob->data->commonscript,"") == 0) {
                    ...
                    sl_doscript_blargs(mob->data->commonscript,"attack",2,&mob->bl,bl);
                }
			} else if(mob->data->subtype == 5) {
				sl_doscript_blargs("mob_ai_ghost","attack",2,&mob->bl,bl);
			} else if(mob->data->subtype == 6) {
				sl_doscript_blargs("mob_ai_pet","attack",2,&mob->bl,bl);
			}
...
int mob_respawn_nousers(MOB *mob) {
    ...
    if(strcmpi(mob->data->commonscript,"") == 0) {
        ...
        sl_doscript_blargs(mob->data->commonscript,"on_spawn",1,&mob->bl);
    }
	//map_addblock(&mob->bl);
...
int mob_find_target(struct block_list *bl, va_list ap) {
	...
	if ((sd->status.state == 1) || (sd->status.state == 2)) return 0;
	...
	if (mob->target) {
		...
		detect = pc_readglobalreg(sd, reg);
		if(detect > 0) {
			mob->target = sd->status.id;
		}
	}
...
int mob_move(struct block_list *bl, va_list ap) {
	...
	if(bl->type==BL_NPC) {
		...
		sd = (USER*)bl;
		if((map[mob->bl.m].show_ghosts && sd->status.state == PC_DIE) || sd->status.state == -1 || sd->status.state == 2) {
