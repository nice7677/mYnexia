...
struct {
	...
	{command_res,"res",98},
	{command_hair,"hair",98},
	{command_face,"face",98},
	{command_checkdupes,"checkdupes",98},
	{command_checkwpe,"checkwpe",98},
	{command_kill,"kick",98},
	{command_killall,"kickall",99},
	{command_deletespell,"del",98},
	{command_xprate,"xprate",99},
	{command_heal,"heal",98},
	{command_level,"level",98},
	{command_randomspawn,"randomspawn17",98},
	{command_drate,"droprate",98},
	{command_luatest,"test",98},
	{command_spell,"spell",98},
	{command_val,"val",98},
	{command_mon,"addmob",99},
	{command_disguise,"disguise",98},
	{command_warp,"warp",50},
	{command_givespell,"givespell",50},
	{command_givespell,"s",50},
	{command_side,"side",50},
	{command_state,"state",50},
	{command_armorcolor,"armorc",50},
	{command_makegm,"makegm",99},
	{command_who,"who",50},
	{command_legend,"legend",50},
	{command_luareload,"reloadlua",50},
	{command_magicreload,"reloadmagic",50},
	{command_lua,"lua",50},
	{command_speed,"speed",50},
	{command_reloaditem,"reloaditem",50},
	{command_reloadmob,"reloadmob",50},
	{command_reloadcreate,"reloadcreate",50},
	{command_reloadspawn,"reloadspawn",50},
	{command_pvp,"pvp",20},
	{command_spellwork,"spellwork",50},
	{command_broadcast,"bc",50},
	{command_luasize,"luasize",50},
	{command_luafix,"luafix",50},
	{command_respawn,"respawn",98},
	{command_kc,"kc",98},
	{command_ban,"ban",98},
	{command_unban,"unban",98},
	{command_blockcount,"blockc",98},
	{command_stealth,"stealth",98},
	{command_ghosts,"ghosts",98},
	{command_unphysical,"unphysical",98},
	{command_immortality,"immortality",98},
	{command_silence,"silence",98},
	{command_shutdowncancel,"a",98},
	{command_shutdown,"gameover",99},
	{command_roll,"roll",0},
	{command_weap,"weap",98},
	{command_shield,"shield",98},
	{command_armor,"armor",98},
	{command_boots,"boots",98},
	{command_mantle,"mantle",98},
	{command_necklace,"necklace",98},
	{command_faceacc,"faceacc",98},
	{command_crown,"crown",98},
	{command_helm,"helm",98},
	{command_gfxtoggle,"gfxtoggle",98},
	{command_weather,"weather",98},
	{command_light,"light",98},
	{command_gm,"gm",1},
	...
	{command_musicn,"nmusic",98},
	{command_musicp,"pmusic",98},
	{command_musicq,"musicq",98},
	{command_sound,"sound",50},
	{command_nsound,"nsound",98},
	{command_psound,"psound",98},
	{command_soundq,"soundq",98},
	{command_nspell,"nspell",98},
	{command_pspell,"pspell",98},
	{command_spellq,"spellq",98},
	{command_reloadboard,"reloadboard",98},
	{command_item,"i",50},
	{command_online,"online",1},
//	{command_metan,"metan",99},
//	{command_reload,"reload",50},
	{command_reloadnpc,"reloadnpc",98},
	{command_reloadrecipe,"reloadrecipe",98},
	{command_reloadmaps, "reloadmaps", 98},
	{command_reloadclass, "reloadclass", 98},
	{command_reloadwarps, "reloadwarps", 98},
	{NULL,NULL,NULL}
};
...
int command_luatest(USER *sd,char* line,lua_State* state) {
	sl_doscript_blargs("onLuaTest", NULL, 1, &sd->bl);
	map[sd->bl.m].spell^=1;
	...
	if(downtimer) {
		...
		for(x=1;x<fd_max;x++) {
			...
			if(session[x] && (tmpsd=session[x]->session_data) && !session[x]->eof) {
				clif_send_timer(tmpsd, 2, 0);//type 2 = count down, 1 = count up
			}
		}
...
int command_shutdown(USER *sd, char* line,lua_State* state) {
	...
	if(!downtimer) {
		...
		*/
		//sprintf(msg,"The storm is approaching, please find a shelter before the time running out..");
		sprintf(msg, "[ATTENTION]  Servers need to restart.. Please stop all activity now and save before the time running out..");
		clif_broadcast("--------------------------------------------------------------------------------",-1);
		clif_broadcast(msg,-1);
		clif_broadcast("--------------------------------------------------------------------------------",-1);
		...
		for(x=1;x<fd_max;x++) {
			...
			if(session[x] && (tmpsd=session[x]->session_data) && !session[x]->eof) {
				d = t_time/1000;
				if ((long)d > 4294967295) {
					tmpsd->disptimertick = 4294967295;
				} else {
					tmpsd->disptimertick = d;
				}

				tmpsd->disptimertype = 2;
				tmpsd->disptimer = timer_insert(1000, 1000, pc_disptimertick, tmpsd->bl.id, 0);
				clif_send_timer(tmpsd, 2, d);//type 2 = count down, 1 = count up
...
int command_silence(USER* sd, char* line,lua_State* state) {
	...
	if(tsd)
	{
	  ...
	  if (tsd->status.id == 2) {
		  clif_sendminitext(sd, "Invalid Target!", 0);
		  return 0;
	  }
...
int command_disguise(USER *sd,char *line,lua_State* state) {
    ...
    //disable 1342 that crashed client -.-
    if (d == 1342 && sd->status.state == 4) {
        clif_sendminitext(sd,"Sorry 1342 not available on state 4",0);
        return -1;
    }
...
int command_legend(USER *sd, char *line,lua_State* state) {
	int icon, color, x;
	char legenddesc[255], legendname[32], target[16];
	USER* tsd = NULL;

	if (sscanf(line,"%15s %d %d %31s %254[^\n]", &target, &icon, &color, &legendname, &legenddesc ) < 5)
		return -1;
	...
	tsd = map_name2sd(target);

	if (tsd != NULL)
	{
		if (strlen(legendname) > 3 && strlen(legenddesc) > 3)
		{
			for (x=0;x<MAX_LEGENDS;x++)
			{
				if(strcmpi(tsd->status.legends[x].name,"") == 0 && strcmpi(tsd->status.legends[x+1].name,"") == 0)
				{
					strcpy(tsd->status.legends[x].text, legenddesc);
					strcpy(tsd->status.legends[x].name, legendname);
					tsd->status.legends[x].icon = icon;
					tsd->status.legends[x].color = color;
					return 0;
				}
			}
		}
		...
		{
			clif_sendminitext(sd, "Legend Name or Description Empty.",0);
		}
	}
	else
	{
		clif_sendminitext(sd, "User not found.",0);
		return -1;
	}
...
int command_face(USER *sd, char *line,lua_State* state) {
	int face;
	int face_color=0;
	if (sscanf(line, "%d %d",&face,&face_color)<1)
		return -1;

	sd->status.face=face;
	sd->status.face_color=face_color;
	clif_sendchararea(sd);
	clif_getchararea(sd);
	return 0;
}
...
int command_mon(USER *sd, char *line,lua_State* state) {
	...
	if (sscanf(line, "%99s %d %d %d", &mname, &start, &end, &replace) < 1)
		return -1;

    if (isnumeric(mname)) {
        id = (unsigned int)strtol(mname, NULL, 10);
    }
    else {
        id = mobdb_id(mname);
    }

    if (id <= 0 || !mobdb_searchexist(id)) {
        clif_sendminitext(sd,"No Monster with that name/id!",0);
        return -1;
    }
...
int command_reloadcreate(USER* sd, char* line,lua_State* state) {
	createdb_term();
	createdb_init();
	nullpo_ret(0, sd);
	clif_sendminitext(sd,"Creation DB Reloaded",strlen("Creation DB Reloaded"));
	return 0;
}
...
int command_nsound(USER *sd, char *line,lua_State* state) {
	...
	sprintf(mini,"Sound is: %d\0",soundfx);
	clif_sendminitext(sd,mini,strlen(mini));	
	return 0;
}
...
int command_reloadmaps(USER *sd, char *line,lua_State* state) {
	...
	clif_sendminitext(sd, "Maps & Warps reloaded!", strlen("Maps & Warps reloaded!"));
	return 0;
}
...
int command_reload(USER *sd, char *line, lua_State* state) {
	...
	command_reloadcreate(sd,line,state);
