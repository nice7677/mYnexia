...
int pc_afktimer(int id, int none) {
	...
	if(map[sd->bl.m].canafk && sd->status.gm_level < 50)//on not afk map and not gm > 50
	{
		if(sd->afk == 1 && sd->afktime >= 60)//already passed 10 minutes
		{
			sl_doscript_blargs("onNoAfkMap", NULL, 1, &sd->bl);
		}
	}

	if(sd->afk == 1 && sd->status.state == 0) {
		sd->totalafktime += 10;
		//clif_sendaction(&sd->bl, 0x10, 0x4E, 305);
...
//	}
	...
	if(sd->afktime >= 180 && sd->status.state == PC_DIE && sd->bl.m != 4 && sd->bl.m != 73) {
        ...
        sl_doscript_blargs("OnAfkWhileDead", NULL, 1, &sd->bl);
	}*/
...
int pc_starttimer(USER *sd) {
	...
	sd->regentimer=timer_insert(1000,1000,pc_regen,sd->bl.id,0);
	...
	sd->thridduratimer = timer_insert(125, 125, bl_thridduratimer, sd->bl.id, 0);
...
int pc_stoptimer(USER *sd) {
	...
	if(sd->thridduratimer) timer_remove(sd->thridduratimer);
	...
	for (x = 0; x < MAX_MAGIC_TIMERS; x++) {
		...
		if (sd->status.dura_aether[x].id > 0) {
			...
			if (sd->status.dura_aether[x].duration > 0) {
				...
				//printf("skillid: %u duration: %u casterid: \n",mid, sd->status.dura_aether[x].duration, sd->status.dura_aether[x].caster_id);
				...
				if (tbl != NULL) {
					...
					//printf("here tbl = null\n");						
					sl_doscript_blargs(magicdb_yname(mid), "while_cast", 1, &sd->bl);
				}
			
				if(sd->status.dura_aether[x].duration <= 0) {
					
					//printf("skillname: %u duration: %u casterid: \n", magicdb_yname(mid), sd->status.dura_aether[x].duration, sd->status.dura_aether[x].caster_id);					
					...
					if(tbl != NULL) {
						//printf("duration 0 tbl != null magicyname: %u\n", magicdb_yname(mid));
...
int bl_thridduratimer(int id, int none) {
	USER *sd = map_id2sd((unsigned int)id);
	struct block_list *tbl = NULL;
	MOB *tmob = NULL;
	int x, mid;
	...
	for (x = 0; x < MAX_SPELLS; x++) {
		if (sd->status.skill[x] > 0) {
			sl_doscript_simple(magicdb_yname(sd->status.skill[x]), "while_passive_125", &sd->bl);
		}
	}
	
	for (x = 0; x < 14; x++) {
		if (sd->status.equip[x].id > 0) {
			sl_doscript_blargs(itemdb_yname(sd->status.equip[x].id), "while_equipped_125", 1, &sd->bl);
		}
	}
		...
		if (sd->status.dura_aether[x].id > 0) {
			...
			if (sd->status.dura_aether[x].duration > 0) {
				if (tbl != NULL) {
					...
					if (health > 0 || tbl->type == BL_PC) {
						sl_doscript_blargs(magicdb_yname(sd->status.dura_aether[x].id), "while_cast_125", 2, &sd->bl, tbl);
					}
				} else {
					sl_doscript_blargs(magicdb_yname(sd->status.dura_aether[x].id), "while_cast_125", 1, &sd->bl);
				}
			}
		}
	}
...
int pc_checklevel(USER *sd) {
	...
	long int lvlxp=0;
	int newlevel=0;
	long int currentExp,remainingExp;
	...
	for(x=sd->status.level;x<99;x++) {
		...
		//printf("level: %u currentexp: %u lvlxp: %u\n", x, sd->status.exp, lvlxp);

		//stop leveling if not choosing path yet
		if(sd->status.level > 5 && path==0) {
		...
		{
			//printf("level: %u currentexp: %u lvlxp: %u\n", x, sd->status.exp, lvlxp);
			break;//just skip check if sd->status.exp < lvlxp)
		}
	}
...
int pc_givexp(USER *sd, unsigned int exp, unsigned int xprate, unsigned int exptype) {
	//int xp_rate=1;
	float insprate;
	unsigned int totalxp, bonusxp;
	unsigned int defaultxp;
	unsigned long tempxp;
	unsigned long difxp;
	unsigned char xpstring[256];
	char regstring[45];
	char exptypestring[10];
	int len, x;
	int stack = 0;
	int bx,by;
	int path;//add path and level checking
	...
	if(sd->status.level == 5 && path == 0)
	{
		len=sprintf(xpstring,"You cannot gain experience without choosing a path first.");
		clif_sendminitext(sd,xpstring,len);
		return 0;
	}
	...
	//inspiration bonus check
	insprate = 0;
	for (x = 0; x < MAX_MAGIC_TIMERS; x++) {
		if (sd->status.dura_aether[x].id > 0) {
			//do checking on timer
			if (sd->status.dura_aether[x].id == 65000)
			{
				insprate = 1;//100%
				break;
			}
			else if (sd->status.dura_aether[x].id == 65001) {
				insprate = 0.5;//50%
				break;
			}
			else if (sd->status.dura_aether[x].id == 65002) {
				insprate = 0.25;//25%
				break;
			}
		}
	...
	bonusxp = 0;
	if(insprate > 0 && exptype == 0)//only type 0 exp gain insp bonus exprate
	{
		bonusxp = ceil(totalxp * insprate);
	...
	sprintf(exptypestring, "Hunt");
	if(exptype == 1)
	{
		sprintf(exptypestring, "Quest");
	}
	else if(exptype == 2)
	{
		sprintf(exptypestring, "Special");
	}

	len=sprintf(xpstring,"You gained %u %s exp", defaultxp, exptypestring);
...
int pc_calcstat(USER *sd) {
	...
	char buf[64];
	struct skill_info *p;
	char baseMight[] = "base_might";
	char baseGrace[] = "base_grace";
	char baseWill[] = "base_will";
	...
	//add mgw from base global registry
	sd->might += pc_readglobalreg(sd, baseMight);
	sd->grace += pc_readglobalreg(sd, baseGrace);
	sd->will += pc_readglobalreg(sd, baseWill);
	...
	sd->ac = (int)(((float)sd->armor * (127.0f / 142462.0f)) + 0.5f) * -1;//make it display minus
	
	if (sd->ac < -85) sd->ac = -85;//max deduction 85%
...
int pc_equipitem(USER *sd, int id) {
	...
	if(!pc_canequipstats(sd,sd->status.inventory[id].id)) {
		clif_sendminitext(sd, "You are too weak to equip that.",0);
		return 0;
	}
...
int pc_getitemmap(USER *sd, int type) {
	FLOORITEM *fl;
	USER* fsd = NULL;
	unsigned char cursedstring[256];
	int len;
	...
	if(!pc_isyours(sd,fl)) {
		fsd = map_id2sd(fl->canloot[0]);
		if(fsd != NULL)
		{
			len=sprintf(cursedstring, "This item is cursed by %s", fsd->status.name);
			clif_sendminitext(sd,cursedstring,len);
		}
		else
		{
...
int pc_useitem(USER *sd, int id) {
	...
	case ITM_EAT:
        ...
        //check commonscript is empty then use identifier
        if(strcmpi(itemdb_commonscript(sd->status.inventory[id].id),"") == 0) {
            sl_doscript_simple(itemdb_yname(sd->status.inventory[id].id),"use",&sd->bl);
        } else {
            sl_doscript_simple(itemdb_commonscript(sd->status.inventory[id].id),"use",&sd->bl);
        }
