...
int addmobspawn(lua_State *state) {
	...
	int x=lua_tonumber(state, 2);
	int y=lua_tonumber(state, 3);
	int id=lua_tonumber(state, 4);
	if(map_isloaded(m)) {
		if(SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `Spawns` (`SpnMapId`, `SpnX`, `SpnY`, `SpnMobId`, `SpnLastDeath`, `SpnStartTime`, `SpnEndTime`, `SpnMobIdReplace`) VALUES(%d, %d, %d, %d, %d, %d, %d, %d)", m, x, y, id, 0, 25, 25, 0))
...
int setmap(lua_State *state) {
	...
	int canafk = lua_tonumber(state, 16);
	int special = lua_tonumber(state, 17);
...
int getclanname(lua_State* state) {
    int clanid = lua_tonumber(state, 1);
	lua_pushstring(state, clandb_name(clanid));
	return 1;
}
...
void sl_init() {
	...
	*/
	...
	lua_register(sl_gstate,"getClanName",getclanname);
	...
	if(luaL_dofile(sl_gstate, "../mYnexiaLua/Developers/sys.lua")) {
...
int sl_reload(lua_State* state) {
	...
	errors += sl_loaddir("../mYnexiaLua/Accepted/", state);
	errors += sl_loaddir("../mYnexiaLua/Developers/", state);

	printf("Lua Script reading finished. %d files read!\n", total_luafiles);
...
int iteml_getattr(lua_State *state, struct item_data *item, char *attrname) {
	...
	else if(!strcmp(attrname, "commonScript")) lua_pushstring(state,item->commonscript);
	...
	else if(!strcmp(attrname, "repairable")) lua_pushboolean(state,item->repairable);
...
int bll_getattr(lua_State *state, struct block_list *bl, char *attrname) {
	...
	else if(!strcmp(attrname,"canAfk")) lua_pushnumber(state,map[bl->m].canafk);
	else if(!strcmp(attrname,"special")) lua_pushnumber(state,map[bl->m].special);
	else return 0; // the attribute was not handled
	return 1;
}
int bll_setattr(lua_State *state, struct block_list *bl, char*attrname) {
	...
	else if(!strcmp(attrname,"canAfk")) map[bl->m].canafk=lua_tonumber(state,-1);
	else if(!strcmp(attrname,"special")) map[bl->m].special=lua_tonumber(state,-1);
	else return 0;
	return 1;
}
...
int mobl_getattr(lua_State *state, MOB *mob, char *attrname) {
	...
	else if(!strcmp(attrname,"commonScript")) lua_pushstring(state, mob->data->commonscript);
	...
	else if(!strcmp(attrname,"con")) lua_pushnumber(state, mob->data->con);
	else if(!strcmp(attrname,"wisdom")) lua_pushnumber(state, mob->data->wisdom);
	...
	else if(!strcmp(attrname,"mountLook")) lua_pushnumber(state, mob->data->mount_look);
		...
		if (damage > 0) {
			if(mob->data->subtype==0) {
                ...
                if(strcmpi(mob->data->commonscript,"") == 0) {
                    ...
                    sl_doscript_blargs(mob->data->commonscript, "on_healed", 2, &mob->bl, bl);
                }
			} else if(mob->data->subtype == 5) {
				sl_doscript_blargs("mob_ai_ghost", "on_healed", 2, &mob->bl, bl);
			} else if(mob->data->subtype == 6) {
				sl_doscript_blargs("mob_ai_pet", "on_healed", 2, &mob->bl, bl);
			}
		}
	} else {
		if (damage > 0) {
			if(mob->data->subtype==0) {
                ...
                if(strcmpi(mob->data->commonscript,"") == 0) {
                    ...
                    sl_doscript_blargs(mob->data->commonscript, "on_healed", 1, &mob->bl);
                }
			} else if(mob->data->subtype == 5) {
				sl_doscript_blargs("mob_ai_ghost", "on_healed", 1, &mob->bl);
			} else if(mob->data->subtype == 6) {
				sl_doscript_blargs("mob_ai_pet", "on_healed", 1, &mob->bl);
			}
		}
	}
...
int mobl_callbase(lua_State* state, MOB* mob) {
	...
	if (bl != NULL) {
        if(strcmpi(mob->data->commonscript,"") == 0) {
            ...
            sl_doscript_blargs(mob->data->commonscript, script, 2, &mob->bl, bl);
        }
		lua_pushboolean(state, 1);
		return 1;
	} else {
        if(strcmpi(mob->data->commonscript,"") == 0) {
            ...
            sl_doscript_blargs(mob->data->commonscript, script, 2, &mob->bl, &mob->bl);
        }
		lua_pushboolean(state, 1);
		return 1;
	}
...
void pcl_staticinit() {
	...
	typel_extendproto(&pcl_type, "getTNL",pcl_gettnl);
...
int pcl_setattr(lua_State *state, USER *sd, char *attrname) {
	...
	else if(!strcmp(attrname,"vendingMsg"))strcpy(sd->vendingmsg,lua_tostring(state, -1));
...
//	}
	...
	else if(!strcmp(attrname,"vendingMsg")) lua_pushstring(state,sd->vendingmsg);
...
int pcl_givexp(lua_State* state, USER *sd) {
	int exptype = 0;//def to hunt
	sl_checkargs(state, "n");
	exptype = lua_tonumber(state, sl_memberarg(2));//0=hunt, 1=quest, 2=special
	pc_givexp(sd, lua_tonumber(state, sl_memberarg(1)),xp_rate, exptype);
}
...
int pcl_stripequip(lua_State *state, USER *sd) {
	...
	{
		//sprintf(itemname, "%s", itemdb_name(sd->status.equip[equipType].id));
...
int pcl_gettnl(lua_State *state, USER *sd) {
    sl_checkargs(state, "n");
    int lvl, tnl, maxtnl;

    lvl = lua_tonumber(state, sl_memberarg(1));

    if (lvl < 99)
    {
        maxtnl=classdb_level(sd->status.class, lvl);
        maxtnl-=classdb_level(sd->status.class, lvl-1);
        //printf("hehe %d, class: %d, lvl: %d \n", maxtnl, sd->status.class, lvl);
        lua_pushnumber(state, maxtnl);
        return 1;
    }
