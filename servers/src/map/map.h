...
struct mobdb_data {
	...
	int sound, mana, physdeduct, con, wisdom, mount_look;
	unsigned int id, mindam, maxdam, drop_count[8], drop[8], drop_rate[8];
	char name[45], yname[45], commonscript[45];
...
struct map_sessiondata {
	...
	int crit, duratimer, physdeduct, scripttimer, scripttick, secondduratimer, thridduratimer, wisdom, bindx, bindy;
	...
	char afkmessage[80], mail[4000], vendingmsg[80];
	...
	int creation_works, creation_item, creation_itemamount, creation_rate, creation_faileditem, creation_failedamount, creation_failrate;
	//boards
