...
int mapif_parse_userlist(int fd) {
	int sfd=RFIFOW(fd,2);
	int jobclass,mark,rank,clan,x,gmlevel;
	char name[32];
	...
	if(SQL_ERROR == SqlStmt_Prepare(stmt,"SELECT `ChaPthId`, `ChaMark`, `ChaClnId`, `ChaName`, `ChaGMLevel` FROM `Character` WHERE `ChaOnline` = 1 AND `ChaHeroes` = 1 GROUP BY ChaName ORDER BY `ChaLevel` DESC, SUM(`ChaMaximumMana` + `ChaMaximumVita`) DESC, `ChaId` ASC")
	|| SQL_ERROR == SqlStmt_Execute(stmt)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_INT, &jobclass, 0, NULL, NULL)
	...
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 4, SQLDT_INT, &gmlevel, 0, NULL, NULL)
	)
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
