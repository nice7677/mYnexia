...
struct map_src_list{
	...
	char title[64];
	unsigned char cantalk, show_ghosts, region, indoor, warpout, bind, canafk;
	unsigned short bgm, light, weather, special;
	char *mapfile;
};
...
int map_src_add(const char *r1) {
	...
	unsigned int sweeptime;
	unsigned short map_bgm,light,weather,special;
	unsigned char cantalk,showghosts,region,indoor,warpout, bind, canafk;
	struct map_src_list *new;
	char map_title[1024], map_file[1024];
	canafk = 0;
	special = 0;
	if (sscanf(r1, "%d,%[^,],%d,%d,%d,%u,%u,%u,%u,%u,%u,%u,%u, %u,%s, %u, %u", &map_id, map_title, &map_bgm, &pvp, &spell, &light, &weather, &sweeptime, &cantalk, &showghosts, &region, &indoor, &warpout, &bind, map_file, &canafk, &special) < 13) {
		return -1;
	}
//[^,],[^\r\n]
...
int map_read() {//int id, const char *title, char bgm, int pvp, int spell, unsigned short light, unsigned short weather, unsigned int sweeptime, unsigned char cantalk, unsigned char showghosts, unsigned char region, unsigned char indoor, unsigned char warpout, const char *map_file) {
	...
	unsigned short bgm;
	unsigned char pvp, spell, light, weather, cantalk, show_ghosts, region, indoor, warpout, bind, canafk, special;
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `MapId`, `MapName`, `MapBGM`, `MapPvP`, `MapSpells`, `MapLight`, `MapWeather`, `MapSweepTime`, `MapChat`, `MapGhosts`, `MapRegion`, `MapIndoor`, `MapWarpout`, `MapBind`, `MapFile`, `MapAfk`, `MapSpecial` FROM `Maps`")
	|| SQL_ERROR == SqlStmt_Execute(stmt)
	...
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 15, SQLDT_UCHAR, &canafk, 0, NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 16, SQLDT_UCHAR, &special, 0, NULL, NULL)
	) {
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
		return 0;
	...
	for (i = 0; i < map_n && SQL_SUCCESS == SqlStmt_NextRow(stmt); i++) {
		sprintf(mappath, "../mYnexiaMaps/Accepted/%s", mapfile);
		fp = fopen(mappath, "rb");
...
int map_reload() {
	...
	for (i = 0; i < map_n && SQL_SUCCESS == SqlStmt_NextRow(stmt); i++) {
		...
		if (map[id].special != special) map[id].special = special;
		...
		if (map[id].canafk != canafk) map[id].canafk = canafk;
...
void do_term(void) {
	...
	printf("mYnexia Map Server Shutdown.\n");
	add_log("Shutdown.\n");
}
...
int change_time_char(int none,int none2) {
	...
	sl_doscript_blargs("mapLight", NULL, 0);
...
int get_time_thing(void) {
	...
	//init outdoor map light based on time
	sl_doscript_blargs("mapLight", NULL, 0);//run set light script :D
...
int do_init(int argc, char **argv) {
	...
	add_log("");
	add_log("mYnexia Map Server Started.\n");
	...
	printf("mYnexia Map Server is \033[1;32mready\033[0m! Listening at %d.\n", map_port);
...
int map_reset_timer(int v1,int v2) {
	...
	diff+=v2;
	if(reset<=250) {
		clif_broadcast("The gods are recasting the magic that sustains the land of mYnexia.", -1);
	}
	if(reset<=0) {
		for(x=0;x<fd_max;x++) {
		if(session[x] && (sd=session[x]->session_data) && !session[x]->eof) {
	...
	if(reset<=60000) { //Less than a minute remaining(gonna mass spell everyone)
		if(diff>=10000) {
			...
			sprintf(msg,"The storm approaching in %d seconds",reset/1000);
			//clif_broadcast("---------------------------------------------------",-1);
		...
		if(diff>=300000) {
			sprintf(msg,"The storm approaching in %d minutes",reset/60000);
			//clif_broadcast("---------------------------------------------------",-1);
			clif_broadcast(msg,-1);
			//clif_broadcast("---------------------------------------------------",-1);
			diff=0;
		...
		if (diff >= 3600000) {
			sprintf(msg,"The storm approaching in %d hours",reset/3600000);
			//clif_broadcast("---------------------------------------------------",-1);
			clif_broadcast(msg,-1);
			//clif_broadcast("---------------------------------------------------",-1);
			diff=0;
