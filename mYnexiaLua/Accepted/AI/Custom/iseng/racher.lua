...
zap = function(mob, target)
	...
	local rangeCheck = false
	local damage = math.random(80000, 160000)
	local aethers = math.random(7,9)
	local range = 12
	local arrow = 6
	local tempDamage = 0
	local critChance = 8
	local minDam = racher.damage(mob, target) * mob.minDam
	local maxDam = racher.damage(mob, target) * mob.maxDam
	...
	if (distance(mob, target) == 0) then
		mob:move()
		if (distance(mob, target) == 0) then
			racher.swap(mob, mobBlocks[math.random(1, #mobBlocks)])
		end
	end
	...
	if (distance(mob, target) <= range and distance(mob, target) > 0) then
		...
		mob:playSound(math.random(368, 369))
		target.attacker = mob.ID
		if (math.random(100) <= 75 and target:hasDuration("snipe") == false) then
			target:sendAnimation(361)
			target:setDuration("snipe", 3000, mob.ID)
			target:playSound(371)
			return
		end
		if (mob:hasDuration("eye_focus") == true and target:hasDuration("turtle_shell") == true) then
			tempDamage = target:removeHealthExtend((target.maxHealth * 0.005), 0, 0, 0, 0, 2)
...
backstep = function(mob, target)
	...
	if (dist == 1) then
		if (canPush(target, mob, 1) == true) then
			...
			racher.swap(mob, target)--swap
			return
		end
		if (math.random(100) <= 10) then
			racher.swap(mob, target)
		end
	end
end,
...
uncast = function(block, caster)
	local minDam = racher.damage(caster, block) * caster.minDam
	local maxDam = racher.damage(caster, block) * caster.maxDam
	local multiplier = 3
	...
	damage = damage + (block.maxHealth * 0.008)
...
uncast = function(block, caster)
	...
	local maxDam = racher.damage(caster, block) * caster.maxDam
	local damage = (math.random(minDam, maxDam))
	damage = damage + (block.baseHealth * 0.2) + (block.health * 0.1)
