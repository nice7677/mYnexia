...
cast = function(player)
	...
	player:spawn(10009, player.x, player.y, 1)
...
on_spawn = function(mob)
	...
	broadcast(mob.m, "A dreadful presence overcomes you.")
...
on_healed = function(mob, healer)
	if (healer.blType == BL_PC) then
		...
		necromancer.on_attacked(mob, healer)
	end
...
move = function(mob, target)
	...
	if (mob.paralyzed or mob.sleep ~= 1) then
		if (#pcBlocks > 0 and mob.registry["souls"] > 0) then
			necromancer.poison(mob, pcBlocks[math.random(#pcBlocks)])
			mob.registry["souls"] = mob.registry["souls"] - 1
		end
	end
	local target
	if (target == nil) then target = pcBlocks[math.random(#pcBlocks)] end
	if (target.m ~= mob.m or target == nil or target == 0 or target.state == 1) then target = pcBlocks[math.random(#pcBlocks)] end

	mob_ai_basic.move(mob, target)

	necromancer.zap(mob, target)
	necromancer_necrosis.cast(mob, target)
	necromancer_curse.cast(mob, target)
	necromancer.poison(mob, target)
	necromancer.summon(mob)
	necromancer.nuke(mob, target)
	necromancer.glare(mob, target)
...
summon = function(mob)
	...
	if (mob.registry["summon"] > os.time()) then return end
	if (mob.registry["souls"] <= 0) then return end
	...
	local check = necromancer.summon_check(target)
	if (check ~= false) then
		local ratio = ((mob.baseHealth - mob.health) / mob.baseHealth) + math.random(0, 1)
		mob:spawn(10001, check[1], check[2], math.ceil(1 * ratio), mob.m)
...
poison = function(mob, target)
	...
	if (mob.registry["poison"] > os.time()) then return end
...
glare = function(mob, target)
	...
	if (target == nil) then return end
	if (target.state == 1) then return end
	if (mob.registry["glare"] > os.time()) then return end
	if (target.blType ~= BL_PC) then return end
...
nuke = function(mob, target)
	...
	if (target == nil) then return end
	if (target.state == 1) then return end
	if (mob.registry["nuke"] > os.time()) then return end
	if (mob.registry["souls"] < 5) then return else mob.registry["souls"] = mob.registry["souls"] - 5 end
...
cast = function(mob, target)
	...
	mob:sendAction(1, 20)
	if (mob.registry["necromancer_regen"] > os.time()) then return end
	if (target == nil) then target = mob end
	if (mob.registry["souls"] <= 0) then return end
...
cast = function(mob, target)
	...
	if (target == nil) then return end
	if (target.state == 1) then return end
	if (mob.registry["necromancer_curse"] > os.time()) then return end
	...
	target:calcStat()
	if (target.blType == BL_PC) then target:sendMinitext("Your spirit has been cursed.") end
...
cast = function(mob, target)
	...
	mob:sendAnimation(105)
	mob:playSound(32)
	mob:playSound(36)
	if (target == nil) then return end
	if (target.state == 1) then return end
	if (mob.registry["necromancer_snare"] > os.time()) then return end
	...
	target:calcStat()
	if (target.blType == BL_PC) then target:sendMinitext("You feel chains binding your very soul.") end
...
recast = function(block) block.snare = true end,
while_cast_250 = function(block)
...
cast = function(mob, target)
	...
	if (target:hasDuration("necromancer_necrosis") == true) then
		...
		if (target:hasDurationID("necromancer_necrosis", mob.ID) == false) then target:setDuration("necromancer_necrosis", duration, mob.ID) end
		if (target:getDuration("necromancer_necrosis") < duration) then target:setDuration("necromancer_necrosis", duration) end
		return
	end
